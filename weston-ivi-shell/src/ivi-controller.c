...
static struct ivi_layout_layer_properties no_layer_props;
static struct ivi_layout_surface_properties no_surface_props;
...
#ifdef SUPPORT_PIXELFORMAT_PROPERTY
                   ...
                   wl_fixed_t opacity)
{
    struct ivisurface *ivisurf = wl_resource_get_user_data(resource);
    (void)client;
    ivi_layout_surface_set_opacity(ivisurf->layout_surface,
                      ...
                      const char *filename)
{
    ...
    if (!(prop = ivi_layout_get_properties_of_surface(ivisurf->layout_surface))) {
                 ...
                 wl_fixed_t opacity)
{
    struct ivilayer *ivilayer = wl_resource_get_user_data(resource);
    (void)client;
    ivi_layout_layer_set_opacity(ivilayer->layout_layer,
				 wl_fixed_to_double(opacity));
}
                          ...
                          struct wl_resource *resource)
{
    ...
    struct ivicontroller_screen *ctrlscrn = NULL;
    struct ivicontroller_screen *next = NULL;
//    uint32_t id_screen = ivi_layout_get_id_of_screen(iviscrn->layout_screen);
    (void)client;
                        ...
                        uint32_t id)
{
    ...
    if (!(prop = ivi_layout_get_properties_of_layer(ivilayer->layout_layer)))
      prop = &no_layer_props;
                        ...
                        struct ivicontroller *controller)
{
    ...
    wl_list_for_each_reverse(ivisurf, &shell->list_surface, link) {
        ...
        id_layout_surface = ivi_layout_get_id_of_surface(layout_surface);
               ...
               uint32_t id_surface)
{
    ...
    ivi_layout_surface_add_notification(layout_surface,
                        ...
                        void *userdata)
{
    ...
    if (!(prop = ivi_layout_get_properties_of_surface(layout_surface)))
      prop = &no_surface_props;
